...
def compress_image_to_480p(image_data):
    try:
        if ',' in image_data:
        ...
        output = BytesIO()
        img.save(output, format='WEBP', quality=55, optimize=True)
        output.seek(0)
        compressed_base64 = base64.b64encode(output.read()).decode('utf-8')
        return 'data:image/webp;base64,' + compressed_base64
...
def clean_old_images():
    try:
        ...
        c = conn.cursor()
        c.execute('UPDATE messages SET image_data = NULL WHERE message_type = "image"')
        c.execute('UPDATE group_messages SET image_data = NULL WHERE message_type = "image"')
        conn.commit()
        affected = conn.total_changes
        conn.close()
        if affected > 0:
            print(f"🧹 已清理 {affected} 条图片")
        return affected
    except Exception as e:
        print(f"❌ 清理图片失败: {e}")
        return 0
...
def call_ai_api(prompt):
    ...
    data = {'model': AI_MODEL, 'messages': [{'role': 'user', 'content': prompt}], 'temperature': 0.7, 'max_tokens': 1000}
    try:
    ...
    if content.startswith('/dl'):
        ...
        if len(parts) == 1:
            ...
            if row:
                ...
                conn.commit()
                conn.close()
                return jsonify({'success': True, 'message': '已删除上一条消息'})
            conn.close()
            return jsonify({'success': False, 'error': '没有可删除的消息'})
        elif len(parts) == 2 and parts[1] == 'all':
            conn = sqlite3.connect(DB_FILE)
            c = conn.cursor()
            c.execute('DELETE FROM messages WHERE sender_id = ? AND receiver_id = ?', (session['user_id'], receiver_id))
            conn.commit()
            conn.close()
            return jsonify({'success': True, 'message': '已删除所有消息'})

    if not receiver_id or (not content and not image_data):
        return jsonify({'error': '参数错误'})
    if msg_type == 'image' and image_data:
        image_data = compress_image_to_480p(image_data)
...
def api_transfer():
    ...
    save_message(0, to_user_id, f'💰 收到来自 {session["username"]} 的转账 {amount} FB' + (f' (备注: {message})' if message else ''),
                 'system')
    save_message(0, session['user_id'], f'💸 已转账 {amount} FB 给 {to_user[1]}' + (f' (备注: {message})' if message else ''),
                 'system')
    return jsonify({
        'success': True,
...
def api_send_group_message():
    ...
    if content.startswith('/dl'):
        ...
        if len(parts) == 1:
            ...
            row = c.fetchone()
            if row:
                c.execute('DELETE FROM group_messages WHERE id = ?', (row[0],))
                conn.commit()
                conn.close()
                return jsonify({'success': True, 'message': '已删除上一条消息'})
            conn.close()
            ...
            conn = sqlite3.connect(DB_FILE)
            c = conn.cursor()
            c.execute('DELETE FROM group_messages WHERE sender_id = ? AND group_id = ?', (session['user_id'], group_id))
            conn.commit()
            conn.close()
            return jsonify({'success': True, 'message': '已删除所有消息'})
...
<div class="app">
    <div class="header">
        <h1>🥓 Hello Chat Pro <span>v2.2</span></h1>
    </div>
...
function openChat(type, id, name) {
    if (currentChatType === type && currentChatId === id) return;
